import { ArrowLeft, Calendar, Hash, TagIcon, User } from "lucide-react"
...
export default async function NewsArticlePage({ params }: { params: { slug: string } }) {
  const [post, settings] = await Promise.all([getPost(params.slug), fetchSettings()])
        ...
        <div className="w-full max-w-[1000px] mx-auto flex flex-col gap-10">
          <section className="relative overflow-hidden border-y border-border bg-background">
            ...
            <div className="relative z-10 px-4 sm:px-8 lg:px-12 py-12 flex flex-col gap-8 text-foreground">
              ...
              <div className="space-y-5 max-w-3xl">
                ...
                <h1 className="text-[30px] sm:text-[42px] lg:text-[54px] leading-tight font-serif">{post.title}</h1>
                <p className="text-base sm:text-lg text-muted-foreground leading-relaxed">{summaryText}</p>
              </div>
          ...
          <div className="w-full max-w-[820px] mx-auto flex flex-col gap-8">
            ...
            <div className="border-t border-[rgba(55,50,47,0.12)] pt-6 flex flex-col gap-4">
              <h3 className="text-sm font-semibold text-[#6B635D] uppercase tracking-[0.3em]">Share this story</h3>
