import { ArrowLeft, Award, BookOpen, Calendar, CheckCircle2, Clock, DollarSign, Download, FileText, GraduationCap, Mail, MapPin, Phone, Users } from "lucide-react";
...
import type { SanityCourse, Settings } from "@/lib/sanity/types";
...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  const description = course.seo?.metaDescription || course.summary || `Learn more about ${course.title} at Data Center College of The Philippines`;
          ...
          <div className="w-full flex flex-col lg:flex-row gap-8">
            <div className="flex-1 flex flex-col gap-8">
              ...
              {course.highlights && course.highlights.length > 0 && (
                <section className="bg-card border border-border rounded-lg p-6 md:p-8">
                  <h2 className="text-2xl font-serif font-semibold text-foreground mb-4">Program Highlights</h2>
              ...
              {course.learningOutcomes && course.learningOutcomes.length > 0 && (
                <section className="bg-card border border-border rounded-lg p-6 md:p-8">
                  <h2 className="text-2xl font-serif font-semibold text-foreground mb-4">Learning Outcomes</h2>
                  <p className="text-muted-foreground mb-4">Upon completion of this program, students will be able to:</p>
                  <ul className="space-y-3">
              ...
              {course.curriculumStructure && course.curriculumStructure.length > 0 && (
                <section className="bg-card border border-border rounded-lg p-6 md:p-8">
                  <h2 className="text-2xl font-serif font-semibold text-foreground mb-6">Curriculum Structure</h2>
                  <div className="space-y-4">
                    {course.curriculumStructure.map((item, idx) => (
                      <div key={idx} className="border-l-2 border-primary pl-4 py-2">
                        {item.term && <h3 className="text-lg font-semibold text-foreground mb-1">{item.term}</h3>}
              ...
              {course.admissionsRequirements && course.admissionsRequirements.length > 0 && (
                <section className="bg-card border border-border rounded-lg p-6 md:p-8">
                  <h2 className="text-2xl font-serif font-semibold text-foreground mb-4">Admissions Requirements</h2>
                  <ul className="space-y-3">
              ...
              {((course.prerequisites && course.prerequisites.length > 0) || (course.prerequisiteNotes && course.prerequisiteNotes.length > 0)) && (
                    ...
                    <div className="mb-4">
                      ...
                      <ul className="space-y-2">
                        {course.prerequisites.map(prereq => (
                          <li key={prereq._id}>
                            <Link href={`/courses/${prereq.slug.current}`} className="text-primary hover:underline">
                              {prereq.title}
                            </Link>
                          </li>
                        ))}
            ...
            <aside className="lg:w-[320px] flex flex-col gap-6">
                    ...
                    <div className="bg-card border border-border rounded-lg p-6 sticky top-4">
                      <h3 className="text-lg font-serif font-semibold text-foreground mb-6">Program Details</h3>
                      <div className="space-y-5">
                        {course.creditHours && (
                          <div>
                            ...
                            <div className="text-base font-medium text-foreground">{course.creditHours}</div>
                          </div>
                        )}
                        ...
                        {course.trainingHours && (
                          <div>
                            ...
                            <div className="text-base font-medium text-foreground">{course.trainingHours}</div>
                          </div>
                        )}
                        ...
                        {course.tuition && (
                          <div>
                            ...
                            <div className="text-base font-medium text-foreground">{course.tuition}</div>
                          </div>
                        )}
                        ...
                        {course.syllabus && (
                          <div>
                            <Button asChild variant="outline" className="w-full" size="sm">
                              <a href={course.syllabus.asset.url} target="_blank" rel="noopener noreferrer">
                      ...
                      <div className="mt-8 space-y-3">
                        {course.cta ? (
                          <Button asChild className="w-full">
                            <a href={course.cta.url || "/#contact"} target="_blank" rel="noopener noreferrer">
                      ...
                      {course.admissionsContact && (
                        <div className="mt-6 pt-6 border-t border-border">
                          <h4 className="text-sm font-semibold text-foreground mb-3">Contact Admissions</h4>
                          <div className="space-y-2 text-sm text-muted-foreground">
                            ...
                            {course.admissionsContact.email && (
                              <div className="flex items-center gap-2">
                                ...
                                <a href={`mailto:${course.admissionsContact.email}`} className="hover:text-primary">
                                  {course.admissionsContact.email}
                                </a>
                              </div>
                            )}
                            {course.admissionsContact.phone && (
                              <div className="flex items-center gap-2">
                                ...
                                <a href={`tel:${course.admissionsContact.phone}`} className="hover:text-primary">
                                  {course.admissionsContact.phone}
                                </a>
                              </div>
                            )}
                      ...
                      {course.applicationDeadlines && course.applicationDeadlines.length > 0 && (
                        <div className="mt-6 pt-6 border-t border-border">
                          <h4 className="text-sm font-semibold text-foreground mb-3">Application Deadlines</h4>
                          <ul className="space-y-2 text-sm text-muted-foreground">
