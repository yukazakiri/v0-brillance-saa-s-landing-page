...
export default function HeroSection({ data, softwareName }: HeroSectionProps) {
  ...
  // Use CMS data if available, otherwise fallback to hardcoded defaults
  ...
  // Use primary hero image from CMS or fallback
  const primaryImage =
    data?.heroLayout === "single"
      ? data?.heroImage
      : data?.heroImages?.find((img) => img.isPrimary)?.image;
  ...
  // Default image if no CMS images are available
      ...
      <main>
              ...
              <div className="flex justify-center gap-4">
                {data?.ctas ? (
                  data.ctas.map((cta, idx) => {
                    const href =
                      cta.url ||
                      (cta.page?._ref ? `/pages/${cta.page._ref}` : "#");
                    return (
                      <Button
                        key={idx}
                        asChild
                        ...
                        className={cn(
                          ...
                          cta.style !== "primary" &&
                            "bg-transparent border border-[#1a3a52] text-[#1a3a52] hover:bg-[#1a3a52]/5 shadow-none",
                        )}
                      >
                        <Link
                          href={href}
              ...
              <div className="absolute inset-0 z-50 opacity-0" ref={emblaRef}>
                <div className="flex w-full h-full touch-pan-y">
                  {stackedImages.map((_, idx) => (
                    <div
                      key={idx}
                      className="flex-[0_0_100%] min-w-0 h-full cursor-grab active:cursor-grabbing"
                    />
                  ))}
                </div>
              </div>
            ...
            <div className="w-full h-full flex justify-center items-center pointer-events-none">
              {!hasImages ? (
                ...
                <div
                  ...
                  style={{
                    transform: "translate3d(0, 0, 0) scale(1) rotateX(20deg)",
                  }}
                >
                ...
                stackedImages.map((img, idx) => {
                  ...
                  if (diff === 0) {
                    ...
                    transformStyle = {
                      transform:
                        "translate3d(60px, 40px, -100px) scale(0.9) rotateY(-8deg) rotateX(20deg)",
                    };
                    ...
                    opacity = 0;
                    transformStyle = {
                      transform: "translate3d(0, 80px, -200px) scale(0.8)",
                    };
                  }
        ...
        <section className="bg-[#F7F5F3] relative z-10 py-16 border-t border-[rgba(26,58,82,0.1)]">
          <div className="m-auto max-w-5xl px-6">
            <h2 className="text-center text-[#1a3a52] text-sm font-bold uppercase tracking-widest mb-12">
            ...
            <div className="mx-auto mt-12 flex max-w-4xl flex-wrap items-center justify-center gap-x-12 gap-y-8 sm:gap-x-16 sm:gap-y-12 opacity-70 grayscale hover:grayscale-0 transition-all duration-500">
              <div className="flex items-center gap-2 font-semibold text-[#1a3a52]">
                <div className="w-8 h-8 bg-[#1a3a52] rounded flex items-center justify-center text-white text-xs">
                  C
                </div>{" "}
                Canvas LMS
              </div>
              <div className="flex items-center gap-2 font-semibold text-[#1a3a52]">
                <div className="w-8 h-8 bg-[#2D73B9] rounded flex items-center justify-center text-white text-xs">
                  Z
                </div>{" "}
                Zoom
              </div>
              <div className="flex items-center gap-2 font-semibold text-[#1a3a52]">
                <div className="w-8 h-8 bg-[#EA4335] rounded flex items-center justify-center text-white text-xs">
                  G
                </div>{" "}
                Google Workspace
              </div>
              <div className="flex items-center gap-2 font-semibold text-[#1a3a52]">
                <div className="w-8 h-8 bg-[#0078D4] rounded flex items-center justify-center text-white text-xs">
                  M
                </div>{" "}
                Microsoft 365
              </div>
