...
export default function CollegeHero({ settings }: CollegeHeroProps) {
  ...
  return (
    <section className="w-full relative min-h-[650px] sm:min-h-[750px] flex flex-col justify-center overflow-hidden border-y border-x border-[rgba(26,58,82,0.12)]">
      {/* BACKGROUND LAYER */}
      <div className="absolute inset-0 z-0">
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-background via-background/80 to-background/20 sm:to-transparent" />
      ...
      <div className="hidden lg:block absolute left-0 top-0 bottom-0 w-12 border-r border-[rgba(26,58,82,0.12)] z-10 overflow-hidden">
        <div className="w-[162px] left-[-58px] top-[-120px] absolute flex flex-col justify-start items-start">
          {Array.from({ length: 200 }).map((_, i) => (
            <div
              key={i}
              className="self-stretch h-4 rotate-[-45deg] origin-top-left outline outline-[0.5px] outline-[rgba(26,58,82,0.08)] outline-offset-[-0.25px]"
            />
          ))}
        </div>
      </div>
      ...
      <div className="relative z-20 w-full px-6 sm:px-10 md:px-20 lg:px-24 flex flex-col justify-center items-start h-full">
        <div className="max-w-3xl flex flex-col items-start gap-8">
          ...
          {/* Heading */}
          <h1 className="font-serif text-primary text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-medium leading-[0.9] tracking-tight animate-in fade-in slide-in-from-bottom-5 duration-700 delay-100">
            Data Center
            <span className="ml-1 text-primary">College</span> <br />
            <span
              className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl block mt-2 text-secondary"
              style={{
            ...
            >
              of the Philippines
              <span
                className="ml-2 text-muted-foreground text-[30px] font-medium leading-tight tracking-wide mt-0.5"
                style={{
                  fontFamily: "ui-serif",
                }}
              >
          ...
          <div className="flex flex-wrap gap-4 mt-2 animate-in fade-in slide-in-from-bottom-6 duration-700 delay-300">
            <Link
              href="/apply"
              className="group px-8 py-4 bg-[#1a3a52] text-white font-medium rounded-md hover:bg-[#1a3a52]/90 transition-all flex items-center gap-2 shadow-lg hover:shadow-xl hover:-translate-y-0.5"
            >
              Apply Now
              <svg
      ...
      <div className="absolute bottom-0 left-0 right-0 z-20 border-t border-x border-[rgba(26,58,82,0.12)] bg-background backdrop-blur-md">
        <div className="grid grid-cols-3 divide-x divide-[rgba(26,58,82,0.12)] max-w-7xl mx-auto border-x border-[rgba(26,58,82,0.12)] lg:border-none">
          {[
            ...
            <div
              key={i}
              className="py-6 px-4 flex flex-col items-center justify-center text-center hover:bg-white/50 transition-colors"
            >
              <span className="font-serif text-2xl sm:text-3xl lg:text-4xl text-[#1a3a52]">
                {stat.value}
              </span>
              <span className="text-[10px] sm:text-xs uppercase tracking-wider text-[#605A57] mt-1 font-medium">
                {stat.label}
              </span>
            </div>
          ))}
      ...
      <div className="absolute bottom-24 right-8 lg:right-20 z-20 hidden sm:flex items-center gap-4 animate-in fade-in duration-1000 delay-500">
        <div className="bg-white/90 backdrop-blur-md border border-[rgba(26,58,82,0.1)] px-4 py-2 rounded-full shadow-lg">
          <span className="text-[#1a3a52] text-xs font-semibold tracking-wide uppercase">
            {campusImages[activeSlide].caption}
          </span>
        </div>
